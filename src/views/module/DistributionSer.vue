<template>
  <div>
    <a-modal :title="title" v-model="visible" style="top:5%;height: 90%;" cancelText="取消" :okText="okText" @ok="handleOk"
      :maskClosable="false">
      <label>基地：</label>
      <a-select v-model="serpost" style="width:100%;">
        <a-select-option v-show="next" v-for="v in baseinfo" :value="v.id" :key="v.id">{{v.baseName}}</a-select-option>
        <a-select-option v-show="!next" v-for="v in baseinfo" :value="v.bid" :key="v.bid">{{v.bName}}</a-select-option>
      </a-select>
    </a-modal>
  </div>
</template>
